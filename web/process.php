...
class Process {
    ...
    global $session;
        ...
        $style = $_POST['Style'];
        $themeScore = $_POST['ThemeScore'];
        $pictureURL = $_POST['pictureURL']; // This should contain the picture URL
        $session->saveScores($composition, $colorfulness, $style, $themeScore, $pictureURL);
    ...
    if (isset($_FILES["fileToUpload"]) && $_FILES["fileToUpload"]["error"] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $targetFile)) {
            ...
            $imageURL = $_SERVER['HTTP_HOST'] . '/' . $targetFile;

            // Retrieve the selected option from the dropdown
            $selectedOption = $_POST["dropdown"];

            // Call the upload function from the Session class
            $session->upload($imageURL, $selectedOption);
			header("Location: " . $session->referrer);
        } else {
            $form->setError("fileToUpload", "Error uploading the file.");
			header("Location: " . $session->referrer);
        }
    } else {
        $form->setError("fileToUpload", "No file was uploaded or an error occurred.");
		header("Location: " . $session->referrer);
    }
}
