...
class Session {
    ...
    function saveScores($composition, $colorfulness, $style, $themeScore, $pictureURL) {
    global $database, $form;
    if ($this->logged_in) {
        if ($database->updateScores($composition, $colorfulness, $style, $themeScore, $pictureURL)) {
            // Update successful, you can do further processing if needed
            // For example, handle the selected option
        } else {
            $form->setError("saveScores", "Error updating the scores.");
        }
    } else {
        $form->setError("saveScores", "User is not logged in.");
    }
}
	...
	function upload($imageURL, $selectedOption) {
    ...
    if ($session->logged_in) {
        ...
        // Update the "art" column in the database with the image URL
        if ($database->insertArt($username, $selectedOption, $imageURL)) {
            // Update successful, you can do further processing if needed
            // For example, handle the selected option
        } else {
            $form->setError("imageUpload", "Error updating the image URL.");
        }
    } else {
        $form->setError("imageUpload", "User is not logged in.");
    }
}
